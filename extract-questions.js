#!/usr/bin/env node
/**
 * Acadmate — Post-UTME Question Extractor
 *
 * Usage:
 *   node extract-questions.js <images-folder> [output.json]
 *
 * Example:
 *   node extract-questions.js ./photos ./unilag-maths-2026.json
 *
 * Setup:
...
For "QUESTIONS" page:
{
  ...
  "questions": [
    ...
    },
    {
      ...
      "text": "In the diagram, a chord of length 8 cm is drawn 3 cm from the centre of a circle. Find the radius.",
      "optionA": "4 cm",
      "optionB": "5 cm",
      "optionC": "6 cm",
      "optionD": "7 cm",
    ...
    },
    {
      ...
      "text": "[DIAGRAM: A coordinate plane with x-axis 0–6 and y-axis 0–10. A line passes through A(1,3) and B(4,9) with two other unlabelled points marked.] Find the gradient of the line AB.",
      "optionA": "1",
      "optionB": "2",
      "optionC": "3",
      "optionD": "4",
    ...
    messages: [
      {
        ...
        content: [
          ...
          {
            type: "text",
            text: "Extract all questions and/or answers from this page. Return ONLY valid JSON, nothing else.",
          },
        ],
      },
    ],
...
// Sends questions in batches of 20. For any whose explanation is wrong or missing,
// Claude writes a correct one. Returns a map of questionNumber → fixed explanation.
async function correctExplanations(questions) {
  ...
  for (let i = 0; i < correctable.length; i += BATCH) {
    ...
    try {
      ...
      const fixed = result.corrections || [];
...
// Maps common OCR/Claude variants to the exact names stored in the DB.

// Keys must be lowercase. Values must exactly match the subject name in the DB.
const SUBJECT_MAP = {
...
function toImportFormat(questions, corrections, yearOverride) {
  return questions
    ...
    .map((q) => ({
      ...
      // internal flags — stripped before final output
...
async function processFolder(folderPath, outputPath, yearOverride) {
  ...
  // Verify explanations match and fix any that don't — single pass
...
────────────────────────────────────────
Usage:
  node extract-questions.js <images-folder> [output.json] [--year YYYY]

Examples:
  node extract-questions.js ./unilag-maths
  node extract-questions.js ./unilag-english   output-english.json
  node extract-questions.js ./photos           unilag-all-2026.json  --year 2026
  node extract-questions.js ./old-booklet      unilag-maths-2022.json --year 2022
...
if (!process.env.ANTHROPIC_API_KEY) {
  ...
  console.error("   Linux/Mac  : export ANTHROPIC_API_KEY=sk-ant-your-key");
  console.error('   PowerShell : $env:ANTHROPIC_API_KEY="sk-ant-your-key"');
  process.exit(1);
}
