...
export default function AdminLeaderboardPage() {
  ...
  useEffect(() => {
    setLoading(true);
    apiClient<LeaderboardEntry[]>(`/api/admin/leaderboard?type=${type}&limit=100`)
      .then(setEntries)
      .catch(() => setEntries([]))
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Tab switcher */}
      <div className="flex rounded-xl overflow-hidden border border-slate-700 w-fit text-sm font-medium">
      ...
      {loading ? (
        <div className="rounded-2xl bg-slate-900 animate-pulse h-80" />
      ) : entries.length === 0 ? (
        <div className="rounded-2xl bg-slate-900 p-12 text-center">
          <div className="text-4xl mb-3">🏆</div>
          <p className="text-slate-400">No results yet for {type === "UTME" ? "JAMB/UTME" : "Post-UTME"}.</p>
        </div>
      ) : (
