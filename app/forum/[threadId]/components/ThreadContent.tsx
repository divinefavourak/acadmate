...
import { createReply } from "@/features/forum/api";
...
export default function ThreadContent({
  ...
  async function handleReply(e: React.FormEvent) {
    ...
    setPostError("");
    try {
      const { reply } = await createReply(thread.id, replyBody.trim());
      setReplies((prev) => [...prev, reply]);
      setReplyBody("");
      setThread((t) => ({ ...t, replyCount: t.replyCount + 1 }));
    } catch {
  ...
  return (
    <div className="max-w-3xl space-y-8">
      ...
      >
        <Link href="/forum" className="hover:text-slate-700 dark:hover:text-slate-300 transition-colors">
          Forum
        </Link>
        <span>/</span>
      ...
      {/* Original post */}
      <article className="rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-white/[0.03] p-6 space-y-4">
      ...
      <section aria-label="Replies">
        <h2 className="font-semibold text-slate-700 dark:text-slate-300 text-sm uppercase tracking-wide mb-4">
        ...
        <div className="space-y-4">
          {replies.map((r) => (
            <div
              key={r.id}
              className="flex items-start gap-3 p-5 rounded-2xl border border-slate-100 dark:border-white/[0.06] bg-slate-50 dark:bg-white/[0.02]"
            >
