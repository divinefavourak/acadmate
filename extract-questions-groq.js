#!/usr/bin/env node
/**
 * Acadmate — Post-UTME Question Extractor (Groq variant)
 *
 * Usage:
 *   node extract-questions-groq.js <images-folder> [output.json] [--year YYYY]
 *
 * Examples:
 *   node extract-questions-groq.js ./unilag-maths
 *   node extract-questions-groq.js ./photos  unilag-2024.json --year 2024
 *
 * Setup:
...
function parseGroqRetryAfter(err) {
  // Groq sets a retry-after header (seconds) on 429 responses
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (retryable && attempt < maxRetries) {
        const wait = parseGroqRetryAfter(err) ?? delay;
...
async function processFolder(folderPath, outputPath, yearOverride) {
  ...
  console.log(`   Correction model : ${TEXT_MODEL}`);
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      if (result.pageType === "MIXED") {
        ...
        // Renumber questions so they are globally sequential across pages.
        // Booklets often restart at Q1 on every page; solutions use absolute numbers.
        const pageQuestions = result.questions || [];
        const firstNum = pageQuestions[0]?.questionNumber ?? 1;
        const needsOffset = firstNum <= questionOffset; // numbers restarted
        if (needsOffset) {
...
if (args.length < 1) {
  console.log(`
Acadmate — Post-UTME Question Extractor (Groq)
───────────────────────────────────────────────
Usage:
  node extract-questions-groq.js <images-folder> [output.json] [--year YYYY]
...
Examples:
  node extract-questions-groq.js ./unilag-maths
  node extract-questions-groq.js ./unilag-english  output-english.json
  node extract-questions-groq.js ./old-booklet     unilag-2022.json --year 2022
